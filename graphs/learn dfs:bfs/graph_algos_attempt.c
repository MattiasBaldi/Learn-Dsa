...
void bfs(Graph* g, int startVertex) {
  ...
  visited[startVertex] = 1; // set the current start-index to 1

  Queue* q = createQueue(); // initialize an empty queue
  enqueue(q, startVertex); // enqueue the start index (add to the back of the array)

  printf("BFS traversal starting from vertex %d: ", startVertex); 

  // if the queue isnt empty continue (continue till it is)
  while (!isQueueEmpty(q)) {
    int vertex = dequeue(q); //take and remove the front one by dequeuing it
    printf("%d ", vertex); 

    // visit all adjacent vertices
    for (int i = 0; i < g->adjSize[vertex]; i++) // stop when the index is higher than the adjacentSize
    {
      int adjVertex = g->adjMatrix[vertex][i];  // set the adjacentVertex 

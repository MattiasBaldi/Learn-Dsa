...
NodeDynamic* newDynamicNode(int data, int childCount) {
  NodeDynamic* n = malloc(sizeof(NodeDynamic));
  n->data = data;
  n->childCount = childCount;

  if (childCount > 0) {
    n->children = calloc(childCount, sizeof(NodeDynamic*));
  } else {
    n->children = NULL;
  }
...
void bfs(char* type, struct NodeDynamic *dNode, struct NodeLinked *lNode) {
  if (strcmp(type, "dynamic") == 0)
  {
    ...
    while (front < rear)
    {
      ...
      // dequeue
      struct NodeDynamic *temp = Queue[front++]; 
  ...
  if (strcmp(type, "linked") == 0)
  {
    ...
    while (front < rear)
    {
      ...
      NodeLinked* temp = Queue[front++]; 

      printf("linked: %d\n", temp->data); 

      // enqueue all children
      NodeLinked* child = temp->firstChild; 
      while(child)
      {
        Queue[rear++] = child; 
...
int main() {
  ...
  // initialize ALL children explicitly
  first->children[0] = newDynamicNode(10, 0);
  first->children[1] = newDynamicNode(11, 0);

  second->children[0] = newDynamicNode(20, 0);
  second->children[1] = newDynamicNode(21, 0);

  third->children[0] = newDynamicNode(30, 0);
  third->children[1] = newDynamicNode(31, 0);

  //dfs("dynamic", root_dynamic, NULL); 
  //bfs("dynamic", root_dynamic, NULL); 
